...
```{r cv}
set.seed(1)
cv.out <- cv.slr(X.adjusted, y, screen.method='wald', cluster.method ='spectral', response.type = 'binary', threshold = NULL, type.measure = 'auc', trace.it = TRUE, plot = TRUE)
```
...
```{r slr}
fit <- slr(X.adjusted, y, screen.method='wald', cluster.method ='spectral', response.type = 'binary', threshold = cv.out$threshold.1se,positive.slope = TRUE)
fit$bp
...
```{r selbal}
train.selbal <- data.frame(y=y,x=log(X.adjusted[,colnames(X.adjusted)=="f_Erysipelotrichaceae_g_unclassified"]) - log(X.adjusted[,colnames(X.adjusted)=="f_Ruminococcaceae_g_Incertae_Sedis"]))
